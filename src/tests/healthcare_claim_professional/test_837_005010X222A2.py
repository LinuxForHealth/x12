...
def test_837_subscriber_drug_in_office(
    x12_837_subscriber_drug_in_office_input,
    x12_837_subscriber_drug_in_office_transaction,
):
    """
    Test case for Example 10: Subscriber Drug Administered in Physician Office

    :param x12_837_subscriber_drug_in_office_input: The full X12 control envelope.
    :param x12_837_subscriber_drug_in_office_transaction: The 837 transaction set.
    """
    with X12ModelReader(x12_837_subscriber_drug_in_office_input) as r:
        model_result = [m for m in r.models()]
        assert len(model_result) == 1
        assert model_result[0].x12() == x12_837_subscriber_drug_in_office_transaction


def test_837_subscriber_properties(x12_837_subscriber_drug_in_office_input):
    """
